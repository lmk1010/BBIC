...
public class MainServiceImpl implements MainService{
	...
	public String uploadexcel(HttpServletRequest request,
		...
		try {
			...
			upload.setFileSizeMax(4096 * 4096);
			// 设置上传文件总量的最大值，最大值=同时上传的多个文件的大小的最大值的和，目前设置为10MB
			upload.setSizeMax(4096 * 4096 * 10);
			...
			for (FileItem item : list) {
				...
				if (item.isFormField()) {
					...
					System.out.println("里面的"+filepath);
					System.out.println("里面的"+savePath);
					// 创建一个文件输出流
	...
	private String makePath(String filename, String savePath) {
		...
		int dir1 = calendar.get(Calendar.YEAR); // 0--15
		int dir2 = calendar.get(Calendar.MONTH); // 0-15
		// 构造新的保存目录
	...
	public TableJsonModel showtb(String excelpath,int pageNumber,int pageSize) {
		...
		try {
			excelpath1 = java.net.URLDecoder.decode(excelpath, "UTF-8");
			TableJsonModel jsonModel = new TableJsonModel();
			
			ArrayList<UserPer> us = new ArrayList<UserPer>();
			int i = 2;
			for(i = 2+(pageNumber-1)*pageSize;i<2+pageNumber*pageSize;i++){
				UserPer u = new UserPer();
				u = PoiExcel.readexcel(excelpath1, i);
				if (u != null) {
					us.add(u);
				} else {
					break;
			...
			int count = 2;
			while (PoiExcel.readexcel(excelpath, count)!=null){
				count++;
			}
			
			jsonModel.setRows(us);
	...
	public TableJsonModel showtbtest(String excelpath) {
		...
		while (i >= 2) {
			UserPer u = new UserPer();
			u = PoiExcel.readexcel(excelpath, i);
			if (u != null) {
				us.add(u);
			} else {
				break;
	...
	public JsonModel saveexcel(String filepath) {
		...
		if (filepath!=null){
			...
			while (i >= 2) {
				UserPer u = new UserPer();
				u = PoiExcel.readexcel(filepath, i);
				if (u != null) {
					us.add(u);
				} else {
					break;
			...
			try{
				int state = userPerDao.addUserPerList(us);
				
				if (state==1){
					return new JsonModel("提交成功!");	
				}else{
					return new JsonModel("提交失败!数据库已有重复数据!");
				}	
			}catch (Exception e) {
			...
			return new JsonModel("先上传！再提交数据");
		}
